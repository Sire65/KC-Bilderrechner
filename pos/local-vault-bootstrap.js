(function(root){
  ...
  async function openDb(){
    ...
    db=await new Promise((resolve,reject)=>{
      ...
      req.onupgradeneeded=()=>{const d=req.result;if(!d.objectStoreNames.contains(DATA))d.createObjectStore(DATA,{keyPath:'key'});if(!d.objectStoreNames.contains(KEYS))d.createObjectStore(KEYS,{keyPath:'id'})};
  ...
  async function loadOrCreateKey(){
    ...
    let tx=d.transaction(KEYS,'readonly'),existing=await idbReq(tx.objectStore(KEYS).get(DEVICE_KEY));await txDone(tx);
    if(existing?.key)return existing.key;
    const key=await crypto.subtle.generateKey({name:'AES-GCM',length:256},false,['encrypt','decrypt']);
    tx=d.transaction(KEYS,'readwrite');tx.objectStore(KEYS).put({id:DEVICE_KEY,key,createdAt:new Date().toISOString(),extractable:false,algorithm:'AES-GCM-256'});await txDone(tx);
    return key;
  }
  async function sealText(value,aad=''){
    await ready;const iv=crypto.getRandomValues(new Uint8Array(12)),params={name:'AES-GCM',iv};if(aad)params.additionalData=enc.encode(aad);
  ...
  async function openText(wrapper,aad=''){
    await ready;if(wrapper?.format!=='KC_LOCAL_VAULT_AES_GCM_V1')throw new Error('VAULT_FORMAT_INVALID');
  ...
  async function persist(key,value){const d=await openDb(),sealed=await sealText(value,`localStorage:${key}`),tx=d.transaction(DATA,'readwrite');tx.objectStore(DATA).put({key,sealed,updatedAt:new Date().toISOString()});await txDone(tx)}
  ...
  Storage.prototype.setItem=function(key,value){if(this===root.localStorage&&protectedKey(key)){key=String(key);value=String(value);memory.set(key,value);schedule(()=>persist(key,value));try{originalRemove.call(this,key)}catch{};return}return originalSet.call(this,key,value)};
  Storage.prototype.removeItem=function(key){if(this===root.localStorage&&protectedKey(key)){key=String(key);memory.delete(key);schedule(()=>removePersisted(key));try{originalRemove.call(this,key)}catch{};return}return originalRemove.call(this,key)};
  ...
  async function persistDirect(key,value){
    ...
    const cipher=await crypto.subtle.encrypt(params,cryptoKey,enc.encode(String(value))),sealed={format:'KC_LOCAL_VAULT_AES_GCM_V1',alg:'AES-256-GCM',iv:bytesToB64(iv),ciphertext:bytesToB64(new Uint8Array(cipher))};
  ...
  async function hydrate(){
    ...
    for(const [key,value] of legacy){if(value==null)continue;if(!memory.has(key))memory.set(key,value);await persistDirect(key,memory.get(key));try{originalRemove.call(root.localStorage,key)}catch{}}
    try{await navigator.storage?.persist?.()}catch{}
    readyResolve(true);
    if(typeof root.dispatchEvent==='function'&&typeof root.CustomEvent==='function')root.dispatchEvent(new root.CustomEvent('kc-local-vault-ready',{detail:{version:VERSION,migratedLegacy:legacy.length,records:memory.size}}));
  }
  ...
  async function audit(){
    ...
    let persistent=false;try{persistent=await navigator.storage?.persisted?.()||false}catch{}
    return {version:VERSION,encryptedRecords:rows.length,memoryRecords:memory.size,plaintextLocalStorageKeys:nativeProtected,keyExtractable:cryptoKey?.extractable===true,persistentStorage:persistent};
  }
