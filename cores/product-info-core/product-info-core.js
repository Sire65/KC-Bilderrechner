(function(root,factory){
  ...
  function unknownAllergens(record){const r=record?.schema==='KC_PRODUCT_INFO_V1'?record:normalize(record);return BIG14.filter(id=>r.allergens?.[id]==='not-checked');}
  function validate(record){
    const r=normalize(record),errors=[];
    if(!r.productId)errors.push('productId fehlt');
    if(r.status==='approved'){
      if(!r.source)errors.push('Quelle fehlt');
      if(!r.approvedAt)errors.push('Freigabedatum fehlt');
      if(!r.approvedBy)errors.push('Freigebende Person fehlt');
      const unchecked=unknownAllergens(r);
      if(unchecked.length)errors.push(`Allergenprüfung unvollständig: ${unchecked.join(', ')}`);
    }
    return {ok:errors.length===0,errors,record:r};
  }
  function canPublish(record){const v=validate(record);return v.ok&&v.record.status==='approved';}
  return Object.freeze({VERSION,BIG14,STATUS,NUTRIENTS,normalize,unknownAllergens,validate,canPublish});
});
