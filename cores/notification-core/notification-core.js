(function(root,factory){
  ...
  async function reconcile(){const rows=localTransactions();if(!rows.length)return {missingRemote:[],missingLocal:[]};const groups=new Map();for(const row of rows){if(!groups.has(row.registerId))groups.set(row.registerId,[]);groups.get(row.registerId).push(row)}let totalRemote=0,totalLocal=0,missingRemote=[],missingLocal=[];for(const [registerId,list] of groups){let reply;try{reply=await fetchJson(`${gateway()}/sync/reconcile`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({registerId,transactionIds:list.map(x=>x.transactionId)})},9000)}catch{return null}const body=reply.body||{};totalRemote+=Number(body.remoteCount||0);totalLocal+=Number(body.localCount||0);for(const id of body.missingRemote||[]){missingRemote.push(id);await storeDelete(FAILOVER.ACK,id);const tx=list.find(x=>x.transactionId===id);if(tx)await queuePut({transactionId:id,registerId,payload:tx,recordHash:tx.recordHash||null,queuedAt:new Date().toISOString(),attempts:0,lastError:null,status:'PENDING'})}if(Array.isArray(body.missingLocal)&&body.missingLocal.length){missingLocal.push(...body.missingLocal);try{const r=await fetchJson(`${gateway()}/sync/transactions?register_id=${encodeURIComponent(registerId)}`,{},10000);const remote=Array.isArray(r.body?.transactions)?r.body.transactions:[],current=localTransactions(),map=new Map(current.map(x=>[x.transactionId,x]));for(const tx of remote)if(tx?.transactionId&&!map.has(tx.transactionId))map.set(tx.transactionId,tx);const merged=[...map.values()].sort((a,b)=>String(a.time||a.endTime||'').localeCompare(String(b.time||b.endTime||'')));if(root.KCStorageVault?.setItemDurable)await root.KCStorageVault.setItemDurable(FAILOVER.TX_KEY,JSON.stringify(merged));else root.localStorage?.setItem(FAILOVER.TX_KEY,JSON.stringify(merged))}catch{}}}FAILOVER.lastReconcile=new Date().toISOString();return {missingRemote,missingLocal,remoteCount:totalRemote,localCount:totalLocal}}
  async function flush(){if(FAILOVER.running)return;FAILOVER.running=true;try{if(root.KCStorageVault?.ready)await root.KCStorageVault.ready;await enqueueMissing();let pending=await queueAll();for(let i=0;i<pending.length;i+=50){const batch=pending.slice(i,i+50).filter(x=>x.payload&&x.status!=='CORRUPT');if(!batch.length)continue;let reply;try{reply=await fetchJson(`${gateway()}/sync/batch`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({transactions:batch.map(x=>x.payload)})},9000)}catch(error){FAILOVER.lastError=error instanceof Error?error.message:String(error);for(const item of batch)await queuePut({...item,attempts:Number(item.attempts||0)+1,lastError:FAILOVER.lastError,status:'PENDING'});break}const results=Array.isArray(reply.body?.results)?reply.body.results:[];for(const result of results){if(result.status==='STORED'||result.status==='ALREADY_STORED')await markAck(result.transactionId,result.status);else if(result.status==='CONFLICT'){const item=batch.find(x=>x.transactionId===result.transactionId);if(item)await queuePut({...item,attempts:Number(item.attempts||0)+1,lastError:'CONFLICT',status:'CONFLICT'})}}FAILOVER.lastError=null;FAILOVER.lastSync=new Date().toISOString()}const left=await queueAll();setHint(left.length,FAILOVER.lastError);if(!left.some(x=>x.status==='CONFLICT'||x.status==='CORRUPT'))await reconcile()}finally{FAILOVER.running=false}}
  async function selfTest(){if(root.KCStorageVault?.ready)await root.KCStorageVault.ready;const id=`SELFTEST-${root.crypto.randomUUID()}`,payload={transactionId:id,registerId:'KC-QUEUE-SELFTEST',recordHash:id};await queuePut({transactionId:id,registerId:payload.registerId,payload,recordHash:id,queuedAt:new Date().toISOString(),attempts:0,lastError:null,status:'SELFTEST'});const raw=(await storeAll(FAILOVER.QUEUE)).find(x=>x.transactionId===id);const read=(await queueAll()).find(x=>x.transactionId===id);const encrypted=!!raw?.sealedPayload&&!Object.prototype.hasOwnProperty.call(raw,'payload');const persisted=read?.payload?.transactionId===id;await storeDelete(FAILOVER.QUEUE,id);const removed=!(await storeAll(FAILOVER.QUEUE)).some(x=>x.transactionId===id);let authProvisioned=false;try{await gatewayAuthConfig();authProvisioned=true}catch{}return {queuePersist:persisted&&removed,queueEncrypted:encrypted,indexedDb:true,authProvisioned,version:FAILOVER.VERSION}}
  ...
  const FailoverSync=Object.freeze({VERSION:FAILOVER.VERSION,start,autoStart:start,syncNow:flush,reconcile,status,selfTest,enqueueMissing,gateway,signGatewayInit});
